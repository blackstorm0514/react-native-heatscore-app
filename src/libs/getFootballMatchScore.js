export const getFootballMatchScore = (timeline, scores) => {
    const matchResult = {
        home_score: '',
        away_score: '',
    }
    ...
    try {
        switch (timeline) {
            case '1st_half':
                ...
                break;
            case '2nd_half':
                matchResult.home_score = scores["3"] && scores["7"] ? parseInt(scores["7"].home) - parseInt(scores["3"].home) : '';
                matchResult.away_score = scores["3"] && scores["7"] ? parseInt(scores["7"].away) - parseInt(scores["3"].away) : '';
                break;
            case '1st_quarter':

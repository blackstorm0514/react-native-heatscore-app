...
export default () => {
  ...
  const renderImageItem = (info: ListRenderItemInfo<ImageSourcePropType>) => (
    <Image
  ...
  const renderOptionItemIcon = (style, icon) => (
    <Icon {...style} name={icon}/>
  );

  const renderOptionItem = (option: ProductOption, index) => (
    <Button
      key={index}
      style={styles.optionItem}
      appearance='ghost'
      size='small'
      accessoryLeft={(style) => renderOptionItemIcon(style, option.icon)}>
      {option.title}
    </Button>
  );
